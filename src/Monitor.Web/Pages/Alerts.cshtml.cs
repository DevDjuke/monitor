    ...
    AuditTrailWriter audit) : PageModel
{
    ...
    private async Task LoadAsync(CancellationToken cancellationToken)
    {
        ...
        AffectedGroups = await alertQuery.Select(x => x.FailureGroupId).Distinct().LongCountAsync(cancellationToken);
        ...
        RecentAlerts = await alertQuery
            ...
            .Select(x => new AlertEventRow(
                x.Id, x.FailureGroupId, x.AlertRule.Name, x.FailureGroup.Category.ToString(), x.FailureGroup.Operation,
                x.FailureGroup.FailureType, x.FailureGroup.Dependency, x.TriggeredAt, x.OccurrencesInWindow, x.Threshold,
                x.WindowStart, x.WindowEnd, x.AcknowledgedAt, x.AcknowledgedBy,
        ...
        Rules = await ruleQuery
            .OrderByDescending(x => x.Enabled).ThenByDescending(x => x.LastTriggeredAt).ThenBy(x => x.Name).Take(100)
            .Select(x => new AlertRuleRow(x.Id, x.FailureGroupId, x.Name, x.FailureGroup.Category.ToString(), x.FailureGroup.Operation,
                x.Threshold, x.WindowMinutes, x.CooldownMinutes, x.Enabled, x.LastEvaluatedAt, x.LastTriggeredAt,
                x.Events.LongCount(e => e.AcknowledgedAt == null)))
            .ToListAsync(cancellationToken);

        Destinations = await db.AlertDeliveryDestinations.AsNoTracking()
            .OrderByDescending(x => x.Enabled).ThenBy(x => x.Name)
            .Select(x => new DestinationRow(x.Id, x.Name, x.Kind, x.EndpointUrl, x.Enabled, x.LastSuccessAt, x.LastFailureAt,
                x.LastFailure, x.Deliveries.LongCount(d => d.Status == AlertDeliveryStatus.Delivered),
                x.Deliveries.LongCount(d => d.Status == AlertDeliveryStatus.DeadLetter)))
            .ToListAsync(cancellationToken);
        ...
        PendingDeliveries = await deliveryQuery.LongCountAsync(x => x.Status == AlertDeliveryStatus.Pending || x.Status == AlertDeliveryStatus.RetryScheduled, cancellationToken);
        DeadLetterDeliveries = await deliveryQuery.LongCountAsync(x => x.Status == AlertDeliveryStatus.DeadLetter, cancellationToken);

        RecentDeliveries = await deliveryQuery.OrderByDescending(x => x.CreatedAt).Take(100)
            .Select(x => new DeliveryRow(x.Id, x.AlertEventId, x.AlertEvent.FailureGroupId, x.Destination.Name, x.Status,
                x.AttemptCount, x.CreatedAt, x.NextAttemptAt, x.LastAttemptAt, x.DeliveredAt, x.ResponseStatusCode, x.LastError))
            .ToListAsync(cancellationToken);
    }

    private IQueryable<FailureAlertEvent> ApplyAlertFilters(IQueryable<FailureAlertEvent> query, DateTimeOffset? since)
    {
        if (since is not null) query = query.Where(x => x.TriggeredAt >= since.Value);
        if (Category is not null) query = query.Where(x => x.FailureGroup.Category == Category.Value);
        query = AlertState switch { "open" => query.Where(x => x.AcknowledgedAt == null), "acknowledged" => query.Where(x => x.AcknowledgedAt != null), _ => query };
        if (!string.IsNullOrWhiteSpace(Search))
        {
            var search = Search;
            query = query.Where(x => x.AlertRule.Name.Contains(search) || x.FailureGroup.Operation.Contains(search) ||
                (x.FailureGroup.FailureType != null && x.FailureGroup.FailureType.Contains(search)) ||
                (x.FailureGroup.Dependency != null && x.FailureGroup.Dependency.Contains(search)) || x.FailureGroup.Fingerprint.Contains(search));
        }
        return query;
    }
    ...
    private IQueryable<FailureAlertRule> ApplyRuleFilters(IQueryable<FailureAlertRule> query)
    {
        ...
        if (Category is not null) query = query.Where(x => x.FailureGroup.Category == Category.Value);
        query = RuleState switch { "enabled" => query.Where(x => x.Enabled), "disabled" => query.Where(x => !x.Enabled), _ => query };
        if (!string.IsNullOrWhiteSpace(Search))
        {
            var search = Search;
            query = query.Where(x => x.Name.Contains(search) || x.FailureGroup.Operation.Contains(search) ||
                (x.FailureGroup.FailureType != null && x.FailureGroup.FailureType.Contains(search)) ||
                (x.FailureGroup.Dependency != null && x.FailureGroup.Dependency.Contains(search)) || x.FailureGroup.Fingerprint.Contains(search));
        }
        return query;
    ...
    private IQueryable<AlertDelivery> ApplyDeliveryFilters(IQueryable<AlertDelivery> query, DateTimeOffset? since)
    {
        if (since is not null) query = query.Where(x => x.CreatedAt >= since.Value);
        if (Category is not null) query = query.Where(x => x.AlertEvent.FailureGroup.Category == Category.Value);
        if (DeliveryStatus is not null) query = query.Where(x => x.Status == DeliveryStatus.Value);
        if (DestinationId is not null) query = query.Where(x => x.DestinationId == DestinationId.Value);
        if (!string.IsNullOrWhiteSpace(Search))
        {
            var search = Search;
            query = query.Where(x => x.Destination.Name.Contains(search) || x.AlertEvent.AlertRule.Name.Contains(search) ||
    ...
    {
        Window = Window?.Trim().ToLowerInvariant() switch { "24h" => "24h", "7d" => "7d", "30d" => "30d", "all" => "all", _ => "24h" };
        AlertState = AlertState?.Trim().ToLowerInvariant() switch { "open" => "open", "acknowledged" => "acknowledged", _ => "all" };
        RuleState = RuleState?.Trim().ToLowerInvariant() switch { "enabled" => "enabled", "disabled" => "disabled", _ => "all" };
        Search = string.IsNullOrWhiteSpace(Search) ? null : Search.Trim();
    }

    private string BuildScopeLabel()
    {
        var parts = new List<string> { Window switch { "24h" => "Last 24 hours", "7d" => "Last 7 days", "30d" => "Last 30 days", _ => "All retained history" } };
        if (Category is not null) parts.Add(Category.Value.ToString());
    ...
    { "24h" => now.AddHours(-24), "7d" => now.AddDays(-7), "30d" => now.AddDays(-30), _ => null };
    ...
    { destination.Name, destination.Kind, destination.EndpointUrl, destination.Enabled, destination.LastSuccessAt, destination.LastFailureAt, destination.LastFailure };
    ...
    private static string RequireHttpEndpoint(string? value, string fieldName, bool requireHttps)
    {
        ...
        if (requireHttps && uri.Scheme != Uri.UriSchemeHttps && !uri.IsLoopback)
            throw new ArgumentException($"{fieldName} must use HTTPS (HTTP is accepted only for loopback development endpoints).");
    ...
    private static string NormalizeEmail(string? value, string fieldName)
    {
        ...
        try { return new MailAddress(email).Address; }
        catch (FormatException) { throw new ArgumentException($"{fieldName} is not a valid email address."); }
    }

    public static string DisplayKind(AlertDeliveryKind kind) => kind switch
    { AlertDeliveryKind.MicrosoftTeams => "Microsoft Teams", AlertDeliveryKind.PagerDuty => "PagerDuty", _ => kind.ToString() };

    public sealed class CreateDestinationInput
    {
        [Required, StringLength(200)] public string Name { get; set; } = string.Empty;
        public AlertDeliveryKind Kind { get; set; } = AlertDeliveryKind.Webhook;
        [StringLength(2000)] public string? EndpointUrl { get; set; }
        [StringLength(2000)] public string? Secret { get; set; }
        [StringLength(320), EmailAddress] public string? EmailRecipient { get; set; }
        [StringLength(500)] public string? SmtpHost { get; set; }
        [Range(1, 65535)] public int SmtpPort { get; set; } = 587;
        [StringLength(320), EmailAddress] public string? SmtpFromAddress { get; set; }
        [StringLength(500)] public string? SmtpUserName { get; set; }
        [StringLength(2000)] public string? SmtpPassword { get; set; }
        public bool SmtpEnableSsl { get; set; } = true;
    }

    public sealed record AlertEventRow(Guid Id, Guid FailureGroupId, string RuleName, string Category, string Operation, string? FailureType,
        string? Dependency, DateTimeOffset TriggeredAt, long OccurrencesInWindow, int Threshold, DateTimeOffset WindowStart,
        DateTimeOffset WindowEnd, DateTimeOffset? AcknowledgedAt, string? AcknowledgedBy, long DeliveredNotifications,
        long DeadLetterNotifications, long PendingNotifications);
    public sealed record AlertRuleRow(Guid Id, Guid FailureGroupId, string Name, string Category, string Operation, int Threshold,
        int WindowMinutes, int CooldownMinutes, bool Enabled, DateTimeOffset? LastEvaluatedAt, DateTimeOffset? LastTriggeredAt, long OpenAlerts);
    public sealed record DestinationRow(Guid Id, string Name, AlertDeliveryKind Kind, string EndpointUrl, bool Enabled,
        DateTimeOffset? LastSuccessAt, DateTimeOffset? LastFailureAt, string? LastFailure, long Delivered, long DeadLetters);
    public sealed record DeliveryRow(Guid Id, Guid AlertEventId, Guid FailureGroupId, string DestinationName, AlertDeliveryStatus Status,
        int AttemptCount, DateTimeOffset CreatedAt, DateTimeOffset NextAttemptAt, DateTimeOffset? LastAttemptAt,
        DateTimeOffset? DeliveredAt, int? ResponseStatusCode, string? LastError);
}
