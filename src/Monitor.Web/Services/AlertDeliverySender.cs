    ...
    IOptions<AlertDeliveryOptions> options)
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var notification = new AlertNotification(
            ...
            new Dictionary<string, object?>
            {
                ...
                ["windowStart"] = alertEvent.WindowStart,
                ["windowEnd"] = alertEvent.WindowEnd
            });

        return await SendAsync(delivery.Destination, notification, cancellationToken);
    }

    public async Task<AlertSendResult> SendBudgetAlertAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        var notification = new AlertNotification(
            alertEvent.Id,
            alertEvent.Level == UsageBudgetAlertLevel.Critical
            ...
            new Dictionary<string, object?>
            {
                ...
                ["periodStart"] = alertEvent.PeriodStart,
                ["periodEnd"] = alertEvent.PeriodEnd,
                ...
                ["observedCostUsd"] = alertEvent.ObservedCostUsd,
                ["observedTokens"] = alertEvent.ObservedTokens,
    ...
    public async Task<AlertSendResult> SendTestAsync(
    ...
    private Task<AlertSendResult> SendAsync(
        AlertDeliveryDestination destination,
    ...
    private async Task<AlertSendResult> SendSlackAsync(
        AlertDeliveryDestination destination,
        AlertNotification notification,
        CancellationToken cancellationToken)
    {
        ...
        var payload = new
        {
            ...
            blocks = new object[]
            {
                ...
                {
                    type = "header",
                    text = new { type = "plain_text", text = Truncate(notification.Title, 150), emoji = true }
                },
                new
                {
                    type = "section",
                    text = new { type = "mrkdwn", text = notification.Summary }
                },
                new
                {
                    type = "context",
                    elements = new[]
                    {
                        new { type = "mrkdwn", text = $"*Event:* `{notification.EventType}` · *ID:* `{notification.Id:D}`" }
                    }
                }
            }
        };
    ...
    private async Task<AlertSendResult> SendTeamsAsync(
        AlertDeliveryDestination destination,
        AlertNotification notification,
        CancellationToken cancellationToken)
    {
    ...
    private async Task<AlertSendResult> SendDiscordAsync(
        AlertDeliveryDestination destination,
        AlertNotification notification,
        CancellationToken cancellationToken)
    {
        ...
        var payload = new
        {
            ...
            embeds = new[]
            {
                new
                {
                    ...
                    footer = new { text = $"{notification.EventType} · {notification.Id:D}" }
                }
            },
    ...
    private async Task<AlertSendResult> SendPagerDutyAsync(
        AlertDeliveryDestination destination,
        AlertNotification notification,
        CancellationToken cancellationToken)
    {
        ...
        var payload = new
        {
            ...
            payload = new
            {
                summary = Truncate($"{notification.Title} — {notification.Summary}", 1024),
                source = "Monitor",
    ...
    private async Task<AlertSendResult> SendEmailAsync(
        AlertDeliveryDestination destination,
        AlertNotification notification,
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            configuration = JsonSerializer.Deserialize<EmailDestinationConfiguration>(protectedJson!);
        }
        catch (JsonException exception)
        {
            return AlertSendResult.PermanentFailure(null, $"Email destination configuration is invalid: {exception.Message}");
        }
        ...
        var recipientValue = destination.EndpointUrl.StartsWith("mailto:", StringComparison.OrdinalIgnoreCase)
            ? destination.EndpointUrl["mailto:".Length..]
        ...
        try
        {
            using var message = new MailMessage
            {
                ...
                Subject = Truncate(notification.Title, 200),
            ...
            using var client = new SmtpClient(configuration.Host, configuration.Port)
            {
                ...
                Timeout = Math.Clamp(_options.RequestTimeoutSeconds, 1, 120) * 1000
            };
        ...
        {
            return AlertSendResult.RetryableFailure(null, "SMTP delivery timed out.");
        }
        catch (SmtpException exception)
        {
            ...
            return retryable
                ? AlertSendResult.RetryableFailure(statusCode == 0 ? null : statusCode, exception.Message)
                : AlertSendResult.PermanentFailure(statusCode == 0 ? null : statusCode, exception.Message);
        }
        catch (FormatException exception)
        {
            return AlertSendResult.PermanentFailure(null, exception.Message);
        }
    ...
    private async Task<AlertSendResult> PostJsonAsync(
        Uri endpoint,
        string eventType,
        object payload,
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            {
                return AlertSendResult.Success((int)response.StatusCode);
            }

            var responseBody = await ReadErrorBodyAsync(response, timeout.Token);
            var error = string.IsNullOrWhiteSpace(responseBody)
                ? $"Destination returned HTTP {(int)response.StatusCode} ({response.ReasonPhrase})."
                : $"Destination returned HTTP {(int)response.StatusCode}: {responseBody}";

            return IsRetryable(response.StatusCode)
                ? AlertSendResult.RetryableFailure((int)response.StatusCode, error)
                : AlertSendResult.PermanentFailure((int)response.StatusCode, error);
        }
        catch (OperationCanceledException) when (!cancellationToken.IsCancellationRequested)
        {
            return AlertSendResult.RetryableFailure(null, "Destination request timed out.");
        }
        catch (HttpRequestException exception)
        {
            return AlertSendResult.RetryableFailure(null, exception.Message);
        }
    }

    private bool TryReadProtectedUri(
        AlertDeliveryDestination destination,
        out Uri? endpoint,
        out AlertSendResult? failure)
    {
        endpoint = null;
        ...
        {
            failure = AlertSendResult.PermanentFailure(null, $"{destination.Kind} webhook URL is invalid or is not HTTPS.");
            endpoint = null;
            return false;
        }
        ...
        out AlertSendResult? failure)
    {
        try
        {
            value = secretProtector.Unprotect(destination.ProtectedSecret);
            failure = null;
            return true;
        }
        ...
        {
            value = null;
            failure = AlertSendResult.PermanentFailure(null, $"Destination secret could not be decrypted: {exception.Message}");
            return false;
        }
    }

    private static AlertSendResult Convert(WebhookSendResult result) =>
        new(result.Succeeded, result.Retryable, result.StatusCode, result.Error);
    ...
    private static string BuildEmailBody(AlertNotification notification)
    {
        ...
        builder.AppendLine();
        builder.AppendLine(notification.Summary);
        builder.AppendLine();
        builder.AppendLine($"Event: {notification.EventType}");
        builder.AppendLine($"ID: {notification.Id:D}");
        builder.AppendLine($"Occurred: {notification.OccurredAt:O}");
        builder.AppendLine();
        builder.AppendLine("Details:");
        foreach (var pair in notification.Details)
        {
    ...
    private sealed record AlertNotification(
        Guid Id,
        string EventType,
        string Title,
        string Summary,
...
public sealed record EmailDestinationConfiguration(
    string Host,
    int Port,
    string FromAddress,
    string? UserName,
